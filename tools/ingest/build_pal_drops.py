...
# Quest-specific and NPC suffixes. These are scenario actors rather than encounters a
# player farms, and they share a base name with a real Pal, so stripping the suffix would
# credit that Pal with a drop only the quest version has.
QUEST_SUFFIX = re.compile(r"_(Quest|Quest_Enemy|Quest_Friend|Avatar)$")
...
def build(version: str) -> dict:
    ...
    for item, droppers in found.items():
        ...
        out[wanted[item]] = sorted(
            droppers.values(),
            key=lambda d: (-d["rate"], -d["max"], d["pal"]))
    ...
    return {
        ...
        "rules": {
            ...
            "quest_actors": "rows with a _Quest/_Avatar suffix are excluded - they share "
                            "a base name with a real Pal and are not farmable encounters",
        },
        "stats": {
            "resources_with_droppers": len(out),
            "resources_total": len(resources),
            "zero_rate_rows_excluded": zero_rate,
            "character_ids_unresolved": len(unresolved),
        },
        "max_named_on_card": MAX_NAMED,
        "drops": dict(sorted(out.items())),
...
def main() -> None:
    ...
    print()
    for res, droppers in data["drops"].items():
